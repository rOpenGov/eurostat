...
\title{\pkg{eurostat}: R Tools for Eurostat Open Data}
...
<<echo=FALSE>>=
  ...
  # Global options
  opts_chunk$set(comment=NA, fig.width=6, fig.height=6, fig.path='fig/')  
@
...
<<2015-manu-install, echo=TRUE, warning=FALSE, error=FALSE, eval=FALSE, tidy=TRUE>>=
...
You can download the complete table of contents of the database with the function \code{get\_eurostat\_toc()}, or use `search\_eurostat()` to make a more focused search over the table of contents. To retrieve data sets for 'disposable income', for instance, use:

<<2015-manu-init, echo=TRUE, warning=FALSE, error=FALSE, eval=TRUE, results=tex>>=
library(eurostat)
income <- search_eurostat("disposable income", type = "dataset")
@

The data type to search for is also specified. The options include \dfn{table}, \dfn{dataset} or \dfn{folder}, referring to different levels of hierarchy in data organization: a 'table' resides in 'dataset' that are stored in a 'folder'. You can focus the search on a selected type.
...
<<2015-manu-getdata, echo=TRUE, warning=FALSE, error=FALSE, eval=TRUE, results=tex>>=
...
%\toprule
<<2015-manu-getdata-table, echo=TRUE, warning=FALSE, error=FALSE, eval=TRUE, results=tex>>=
...
This looks like Table~\ref{tab:getdatatable}
...
\begin{center}
<<2015-manu-mapexample, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE, fig.width=7, fig.height=7, fig=TRUE, dpi=300, cache=TRUE>>=
# Clean the cache
...
\setkeys{Gin}{width=0.5\textwidth}
\begin{center}
<<2015-manu-searchdata, echo=FALSE, warning=FALSE, error=FALSE, eval=TRUE, fig=TRUE, fig.width=7, fig.height=7, dpi=300, cache=TRUE, fig.show='asis'>>=
...
<<2015-manu-searchdata, echo=TRUE, warning=FALSE, error=FALSE, eval=TRUE, results=tex>>=
data(efta_countries)
...
\setkeys{Gin}{width=0.5\textwidth}
\begin{center}
<<2015-manu-roadaccidents, echo=FALSE, warning=FALSE, error=FALSE, eval=TRUE, fig=TRUE, fig.width=7, fig.height=7, dpi=300, out.width=5cm, fig.align='center', fig.show='asis'>>=
...
\setkeys{Gin}{width=0.5\textwidth}
\begin{center}
<<2015-manu-bmi, echo=FALSE, warning=FALSE, error=FALSE, eval=TRUE, fig=TRUE, fig.width=7, fig.height=7, out.width='.5\\linewidth', fig.align='center', fig.show='asis'>>=
library(dplyr)
...
tmp1 %>%
  ...
  theme(legend.position="top") + ggtitle("Body mass index (BMI) by sex and age")+ylab("% of population")+xlab("Age group")+scale_fill_brewer(type = "div")
@
\end{center}
\caption{BMI caption}
\label{fig:bmi}
