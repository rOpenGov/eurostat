...
```{r RJournal_map_example, warning=FALSE, message=FALSE, echo=FALSE, eval=FALSE}
# Let us first download the data and convert the time column into numeric format
library(eurostat)
...
# Extract the information on country in question.
# (First two character in region string mark the country)
...
# Plot the data at the regional level and color the lines using country names:
library(ggplot2)
...
# Labelling the data
# Remove the cntry and countrycode columns that were added manually
...
# Plot four countries in alphabetical order in their own facets with
# region names.
...
plotrix `r citep(citation("plotrix"))` package, support by 
