...
facilities, there are inevitably some changes that affect the `eurostat`
package as well. One of those is the removal of old `.dic` objects that
have been used in translating Eurostat variable codes into cleartext
labels in English, French or German.
...
function that simply downloaded `dimlst.dic`, sort of ‘master file’ of
all available codes and their cleartext labels, and used that to label
all Eurostat datasets. Now labeling is done by downloading a Concept
Scheme file for each individual dataset and using that information to
give labels to the dataset in the desired language.

## Comparison of old and new

An example of the old English version dimlst.dic file (downloaded
2023-12-18 from
[here](https://ec.europa.eu/eurostat/estat-navtree-portlet-prod/BulkDownloadListing?sort=1&dir=dic%2Fen)),
10 first rows:
...
The apparent benefit of XML presentation is that all language versions
can be found in the same file. This makes the files a bit larger than
old .tsv files but for individual datasets the size is still manageable.

Concept id’s or Ref id’s (“unit”, “UNIT”) can be used to look up further
classifications in Codelists. In the old .dic metaphor all definition
files were “dictionaries” where `dimlst.dic` was a special case, a
dictionary of dictionaries, whereas in the new metaphor there is more of
a hierarchy of definitions. For example in the case of units all
available English labels can be downloaded in JSON-stat and TSV formats:
...
However, the new TSV files and the old .dic files are virtually
identical:
...
file to create a list that is similar to the old `dimlst.dic` object to
...
We can see that there are more unique codes than labels and that some
...
To make the data.frame similar to the one that we would get from reading
...
Clearly, the two objects have different types (the former is a
...
However, `dimlst.dic` has 623 rows while `new_df` object has 593 rows
...
To summarize, the old `dimlst.dic` file has 38 codes that are not found
in the `ALL_CONCEPTSCHEMES.xml` file. The new `ALL_CONCEPTSCHEMES.xml`
has 8 codes that are not found in the old `dimlst.dic`.

For more information about the different fields, let’s print the codes
...
Of these, “FIELDID / Agricultural product” seems almost like an input
...
Duplicates exist both in the `code_name` column or in the `full_name`
column.
...
…or due to actual differences (?) in the fields, although there could
...
Changes in methods of delivering metadata affect, naturally, all end
...
should not be of concern for users.

This document is written mainly as a future reference for ourselves if
we might at some point be wondering why something that was possible
before is not possible anymore, or for users who have had scripts
relying on functions like
[`label_eurostat_vars()`](https://ropengov.github.io/eurostat/reference/label_eurostat.md).

Feel free to open an issue or a pull request in Github if there are any
suggestions or corrections you would like to make.
