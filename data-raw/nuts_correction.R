...
check_unique_values <- function(df) {
  ...
  if ( any (df$n)>1) return(TRUE) else return(FALSE)
}
...
correct_nuts_labelling <- function ( dat ) { 
  ...
  corrected_dataset <- so_far_joined %>%
    ...
    fill ( -one_of (c("geo", "time", "country_code", "years", "country",
  ...
  corrected_dataset2 <- corrected_dataset %>% select (-n) %>%
    ungroup() %>%
    filter ( !is.na(change)) %>%
  ...
  if ( any(corrected_dataset2$n>1) ) {
    ...
    corrected_dataset3 <- corrected_dataset2 %>%
      select (-n) %>% add_count ( geo, time, indicator ) %>%
      filter (n>1)
...
check_spread <- function( dat ) {
  dat %>% filter ( indicator == unique(indicator)[1]) %>%
